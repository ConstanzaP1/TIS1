...
// Consulta para obtener datos de la tabla socket_placa que no sean NULL
$query = "
    SELECT p.id_hardware, 
           sp.socket_placa
    FROM hardware p
    LEFT JOIN socket_placa sp ON p.id_hardware = sp.id_hardware
    WHERE sp.socket_placa IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <div id="tabla">
        <div class="row">
            <div class="col">
                <h2>Socket Placa madre</h2>
                <table class="table table-bordered mt-4">
                    <thead>
                        <tr>
                            <th>Socket Placa madre</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Socket placa madre</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="socket_placa">

        <div class="mb-3">
            <label for="socket_placa" class="form-label">Socket Placa madre</label>
            <input type="text" name="socket_placa" class="form-control" id="socket_placa" required>
        </div>
