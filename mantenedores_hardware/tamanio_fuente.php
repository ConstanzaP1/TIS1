...
// Consulta para obtener datos de la tabla tamanio_fuente que no sean NULL
$query = "
    SELECT p.id_hardware, 
           tf.tamanio_fuente
    FROM hardware p
    LEFT JOIN tamanio_fuente tf ON p.id_hardware = tf.id_hardware
    WHERE tf.tamanio_fuente IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Tamaño Fuente </h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="tamanio_fuente">

        <div class="mb-3">
            <label for="tamanio_fuente" class="form-label">Tamaño Fuente</label>
            <input type="text" name="tamanio_fuente" class="form-control" id="tamanio_fuente" required>
        </div>
