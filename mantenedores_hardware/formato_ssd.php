...
// Consulta para obtener datos de la tabla formato_ssd que no sean NULL
$query = "
    SELECT p.id_hardware, 
           fs.formato_ssd
    FROM hardware p
    LEFT JOIN formato_ssd fs ON p.id_hardware = fs.id_hardware
    WHERE fs.formato_ssd IS NOT NULL
";
...
<div class="container mt-5">
                ...
                <table class="table table-bordered mt-4">
                    ...
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $row['formato_ssd']; ?></td>
                                <td>
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de formato ssd </h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="formato_ssd">

        <div class="mb-3">
            <label for="formato_ssd" class="form-label">formato ssd</label>
            <input type="text" name="formato_ssd" class="form-control" id="formato_ssd" required>
        </div>
