...
// Consulta para obtener datos de la tabla tipo_cableado que no sean NULL
$query = "
    SELECT p.id_hardware, 
           tc.tipo_cableado
    FROM hardware p
    LEFT JOIN tipo_cableado tc ON p.id_hardware = tc.id_hardware
    WHERE tc.tipo_cableado IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Tipo Cableado</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="tipo_cableado">

        <div class="mb-3">
            <label for="tipo_cableado" class="form-label">Tipo Cableado</label>
            <input type="text" name="tipo_cableado" class="form-control" id="tipo_cableado" required>
        </div>
