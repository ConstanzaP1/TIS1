...
// Consulta para obtener datos de la tabla tamanio_max_gabinete que no sean NULL
$query = "
    SELECT p.id_hardware, 
           tmg.tamanio_max_gabinete
    FROM hardware p
    LEFT JOIN tamanio_max_gabinete tmg ON p.id_hardware = tmg.id_hardware
    WHERE tmg.tamanio_max_gabinete IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <div id="tabla">
        <div class="row">
            <div class="col">
                <h2>Tamaño Placa Gabinete</h2>
                <table class="table table-bordered mt-4">
                    <thead>
                        <tr>
                            <th>Tamaño placa gabinete</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php while ($row = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $row['tamanio_max_gabinete']; ?></td>
                                <td>
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Tamaño Placa Gabinete </h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="tamanio_max_gabinete">

        <div class="mb-3">
            <label for="tamanio_max_gabinete" class="form-label">Tamaño Placa Gabinete</label>
            <input type="text" name="tamanio_max_gabinete" class="form-control" id="tamanio_max_gabinete" required>
        </div>
