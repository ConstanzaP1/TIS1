...
// Consulta para obtener datos de la tabla slot_memoria_placa que no sean NULL
$query = "
    SELECT p.id_hardware, 
           smp.slot_memoria_placa
    FROM hardware p
    LEFT JOIN slot_memoria_placa smp ON p.id_hardware = smp.id_hardware
    WHERE smp.slot_memoria_placa IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de slot memoria placa</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="slot_memoria_placa">

        <div class="mb-3">
            <label for="slot_memoria_placa" class="form-label">Slot memoria placa</label>
            <input type="text" name="slot_memoria_placa" class="form-control" id="slot_memoria_placa" required>
        </div>
