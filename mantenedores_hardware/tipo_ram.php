...
// Consulta para obtener datos de la tabla tipo_ram que no sean NULL
$query = "
    SELECT p.id_hardware, 
           tr.tipo_ram
    FROM hardware p
    LEFT JOIN tipo_ram tr ON p.id_hardware = tr.id_hardware
    WHERE tr.tipo_ram IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Tipo Ram</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="tipo_ram">

        <div class="mb-3">
            <label for="tipo_ram" class="form-label">Tipo Ram</label>
            <input type="text" name="tipo_ram" class="form-control" id="tipo_ram" required>
        </div>
