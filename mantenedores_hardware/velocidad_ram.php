...
// Consulta para obtener datos de la tabla velocidad_ram que no sean NULL
$query = "
    SELECT p.id_hardware, 
           vr.velocidad_ram
    FROM hardware p
    LEFT JOIN velocidad_ram vr ON p.id_hardware = vr.id_hardware
    WHERE vr.velocidad_ram IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Velocidad RAM</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="velocidad_ram">

        <div class="mb-3">
            <label for="velocidad_ram" class="form-label">Velocidad RAM</label>
            <input type="text" name="velocidad_ram" class="form-control" id="velocidad_ram" required>
        </div>
