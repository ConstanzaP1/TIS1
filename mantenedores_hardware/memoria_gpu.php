...
// Consulta para obtener datos de la tabla memoria_gpu que no sean NULL
$query = "
    SELECT p.id_hardware, 
           mg.memoria_gpu
    FROM hardware p
    LEFT JOIN memoria_gpu mg ON p.id_hardware = mg.id_hardware
    WHERE mg.memoria_gpu IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <button type="button" id="botonVolverInicio" class="btn btn-secondary" onclick="window.location.href='../admin_panel.php.php';">Volver a Inicio</button>

    <!-- Formulario oculto inicialmente -->
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de memoria GPU</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="memoria_gpu">

        <div class="mb-3">
            <label for="memoria_gpu" class="form-label">Memoria GPU</label>
            <input type="text" name="memoria_gpu" class="form-control" id="memoria_gpu" required>
        </div>
