...
// Consulta para obtener datos de la tabla formato_placa que no sean NULL
$query = "
    SELECT p.id_hardware, 
           fp.formato_placa
    FROM hardware p
    LEFT JOIN formato_placa fp ON p.id_hardware = fp.id_hardware
    WHERE fp.formato_placa IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Formato Placa</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="formato_placa">

        <div class="mb-3">
            <label for="formato_placa" class="form-label">Formato Placa</label>
            <input type="text" name="formato_placa" class="form-control" id="formato_placa" required>
        </div>
