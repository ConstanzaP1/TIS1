...
// Consulta para obtener datos de la tabla capacidad_ram que no sean NULL
$query = "
    SELECT p.id_hardware, 
           cr.capacidad_ram
    FROM hardware p
    LEFT JOIN capacidad_ram cr ON p.id_hardware = cr.id_hardware
    WHERE cr.capacidad_ram IS NOT NULL
";
...
<div class="container mt-5">
    ...
    <form action="ingresar_hardware.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Capacidad Ram</h1>
        <!-- Campo oculto para seleccionar automáticamente-->
        <input type="hidden" name="tipo_hardware" value="capacidad_ram">

        <div class="mb-3">
            <label for="capacidad_ram" class="form-label">Capacidad Ram</label>
            <input type="text" name="capacidad_ram" class="form-control" id="capacidad_ram" required>
        </div>
