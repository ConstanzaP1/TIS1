...
// Consulta para obtener datos de la tabla bateria_notebook que no sean NULL
$query = "
    SELECT p.id_notebook, 
           bn.bateria_notebook
    FROM notebook p
    LEFT JOIN bateria_notebook bn ON p.id_notebook = bn.id_notebook
    WHERE bn.bateria_notebook IS NOT NULL
";
...
<div class="container mt-5">
    <div class="container" id="tabla">
        <div class="row">
            <div class="col">
                <h2>Batería notebook</h2>
                <table class="table table-bordered">
                    <thead>
                        <tr>
                            <th>Batería notebook</th>
                            <th>Acciones</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php while ($rowCategoria = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $rowCategoria['bateria_notebook']; ?></td>
                                <td>
    ...
    <button type="button" id="botonAgregar" class="btn btn-primary" onclick="mostrarFormulario()">Agregar Batería Notebook</button>
    ...
    <form action="ingresar_notebook.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Batería notebook</h1>
        <!-- Campo oculto para seleccionar automaticamente "Batería notebook" -->
        <input type="hidden" name="tipo_notebook" value="bateria_notebook">

        <div class="mb-3">       
            <label for="bateria_notebook" class="form-label mt-3">Batería notebook</label>
            <input type="text" name="bateria_notebook" class="form-control" id="bateria_notebook" required>
        </div>

        <!-- Contenedor para alinear los botones -->
        <div class="d-flex justify-content-between mt-3">
            <!-- Botón de guardar -->
            <button type="submit" class="btn btn-success">Guardar Batería notebook</button>
