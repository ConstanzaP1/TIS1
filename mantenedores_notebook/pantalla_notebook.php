...
// Consulta para obtener datos de la tabla pantalla_notebook que no sean NULL
$query = "
    SELECT p.id_notebook, 
           pn.pantalla_notebook
    FROM notebook p
    LEFT JOIN pantalla_notebook pn ON p.id_notebook = pn.id_notebook
    WHERE pn.pantalla_notebook IS NOT NULL
";
...
<div class="container mt-5">
                ...
                <table class="table table-bordered mt-4">
                    ...
                    <tbody>
                        <?php while ($rowCategoria = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $rowCategoria['pantalla_notebook']; ?></td>
                                <td>
    ...
    <form action="ingresar_notebook.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso de Pantalla notebook</h1>
        <!-- Campo oculto para seleccionar automaticamente "Pantalla notebook" -->
        <input type="hidden" name="tipo_notebook" value="pantalla_notebook">

        <div class="mb-3">       
            <label for="pantalla_notebook" class="form-label mt-3">Pantalla notebook</label>
            <input type="text" name="pantalla_notebook" class="form-control" id="pantalla_notebook" required>
        </div>
