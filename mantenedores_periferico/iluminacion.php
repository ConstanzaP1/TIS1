...
// Consulta para obtener datos de la tabla iluminacion que no sean NULL
$query = "
    SELECT p.id_periferico, 
           i.iluminacion
    FROM periferico p
    LEFT JOIN iluminacion i ON p.id_periferico = i.id_periferico
    WHERE i.iluminacion IS NOT NULL
";
...
<div class="container mt-5">
                ...
                <table class="table table-bordered mt-4">
                    ...
                    <tbody>
                        <?php while ($rowCategoria = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $rowCategoria['iluminacion']; ?></td>
                                <td>
    ...
    <form action="ingresar_periferico.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso Iluminacion</h1>
        <!-- Campo oculto para seleccionar automaticamente -->
        <input type="hidden" name="tipo_periferico" value="iluminacion">

        <div class="mb-3">       
            <label for="iluminacion" class="form-label mt-3">Iluminacion</label>
            <input type="text" name="iluminacion" class="form-control" id="iluminacion" required>
        </div>
