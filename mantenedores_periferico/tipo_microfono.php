...
// Consulta para obtener datos de la tabla tipo_microfono que no sean NULL
$query = "
    SELECT p.id_periferico, 
           cm.tipo_microfono
    FROM periferico p
    LEFT JOIN tipo_microfono cm ON p.id_periferico = cm.id_periferico
    WHERE cm.tipo_microfono IS NOT NULL
";
...
<div class="container mt-5">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php while ($rowCategoria = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $rowCategoria['tipo_microfono']; ?></td>
                                <td>
    ...
    <form action="ingresar_periferico.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        ...
        <div class="mb-3">       
            <label for="tipo_microfono" class="form-label mt-3">Tipo Micrófono</label>
            <input type="text" name="tipo_microfono" class="form-control" id="tipo_microfono" required>
        </div>

        <!-- Contenedor para alinear los botones -->
        <div class="d-flex justify-content-between mt-3">
            <!-- Botón de guardar -->
            <button type="submit" class="btn btn-success">Guardar tipo Micrófono</button>
