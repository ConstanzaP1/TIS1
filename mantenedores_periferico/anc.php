...
// Consulta para obtener datos de la tabla anc que no sean NULL
$query = "
    SELECT p.id_periferico, 
           a.anc
    FROM periferico p
    LEFT JOIN anc a ON p.id_periferico = a.id_periferico
    WHERE a.anc IS NOT NULL
";
...
<div class="container mt-5">
                ...
                <table class="table table-bordered mt-4">
                    ...
                    <tbody>
                        <?php while ($rowCategoria = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $rowCategoria['anc']; ?></td>
                                <td>
    ...
    <form action="ingresar_periferico.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        <h1 class="mb-4">Ingreso Anc</h1>
        <!-- Campo oculto para seleccionar automaticamente -->
        <input type="hidden" name="tipo_periferico" value="anc">

        <div class="mb-3">       
            <label for="anc" class="form-label mt-3">Anc</label>
            <input type="text" name="anc" class="form-control" id="anc" required>
        </div>
