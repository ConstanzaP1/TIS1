...
// Consulta para obtener datos de la tabla tipo_audifono que no sean NULL
$query = "
    SELECT p.id_periferico, 
           ca.tipo_audifono
    FROM periferico p
    LEFT JOIN tipo_audifono ca ON p.id_periferico = ca.id_periferico
    WHERE ca.tipo_audifono IS NOT NULL
";
...
<div class="container mt-5">
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php while ($rowCategoria = mysqli_fetch_assoc($result)) : ?>
                            <tr>
                                <td><?php echo $rowCategoria['tipo_audifono']; ?></td>
                                <td>
    ...
    <button type="button" id="botonAgregar" class="btn btn-primary" onclick="mostrarFormulario()">Agregar Tipo Audifono</button>
    ...
    <form action="ingresar_periferico.php" method="POST" id="formulario" style="display: none;" class="mt-4">
        ...
        <input type="hidden" name="tipo_periferico" value="tipo_audifono">

        <div class="mb-3">       
            <label for="tipo_audifono" class="form-label mt-3">Tipo Audifono</label>
            <input type="text" name="tipo_audifono" class="form-control" id="tipo_audifono" required>
        </div>

        <!-- Contenedor para alinear los botones -->
        <div class="d-flex justify-content-between mt-3">
            <!-- Botón de guardar -->
            <button type="submit" class="btn btn-success">Guardar tipo Audifono</button>
