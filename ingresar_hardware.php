...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_query($conexion, $queryHardware)) {
        ...
        elseif ($tipo_hardware == 'slots_memoria_placa') {
            $slots_memoria_placa = $_POST['slots_memoria_placa'];
            // Insertar en las tablas asociadas para procesadores
            $querySlots_memoria_placa = "INSERT INTO slots_memoria_placa (id_hardware, slots_memoria_placa) VALUES ('$id_hardware', '$slots_memoria_placa')";

            if (mysqli_query($conexion, $querySlots_memoria_placa)) {
                echo "Ingreso exitoso.";
            } else {
                echo "Error al insertar." . mysqli_error($conexion);
            }
